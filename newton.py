...
if __name__ == '__main__':
    ...
    z=17
    sr_fx=getZ(zBound=z)
    sr_x=(np.arange(len(sr_fx)))

    Nx = get_Newton_inter(list(sr_x), sr_fx)  # 获得插值函数
    # 测试用例
    tmp_x = np.linspace(sr_x.min(), sr_x.max()-1, 100)
    ...
    ax1 = plt.subplot(111)
    plt.sca(ax1)
    plt.plot(sr_x[:z-1], sr_fx[:z-1], linestyle='', marker='o', color='b')
